...
mcore1plot <- johnson_neyman(mcoreIRTvar, pred = "Eth_Frac_mc", modx = "IRTscores_mc", 
               ...
               alpha = .1, title = "Conditional Effect of Ethnic Fractionalisation on Social Trust") 


sim_slopes(mcoreIRTvar, pred = "Eth_Frac_mc", modx = "IRTscores_mc", johnson_neyman = TRUE)
...
stargazer(mbasevar, mcorevar, mbaseIRTvar, mcoreIRTvar, align = TRUE, 
...
stargazer(mcoreIRTvar, mIRTattimvar, align = TRUE, 
          star.cutoffs = c(0.05, 0.01, 0.001))
...
#rerun base model 
mbasenat <- lmer(soc_trst ~ Eth_Frac_mc + essround + (1 + Eth_Frac_mc | reg_code:cntry) + 
                (1 |cntry), 
...
stargazer(mbasenat, mcorenat, mbaseIRTnat, mcoreIRTnat, align = TRUE, 
          star.cutoffs = c(0.05, 0.01, 0.001))
...
stargazer(mcoreIRTnat, mIRTattimnat, align = TRUE, 
          star.cutoffs = c(0.05, 0.01, 0.001))
#-----------------using interflex to model the interaction----------------------
#full model IRT  
controls1 <- c("hinctnt", "eduyrs", "avgeduyrs", "res_turn",
               "sin_par_hh", "unemp_rate", "crmvct", 
...
min(as.numeric(census_2001_controls$area), na.rm = TRUE)
